...
function initSubModule() {
    ...
    try {
        if (config.subscriber) {
            console.log(`加载配置 subscriber (${config.subscriber}) 模块`);
            subModule = require(config.subscriber);
        } else {
            const defModulePath = path.join(process.cwd(), 'subscriber.js');
            console.log(`加载默认 subscriber (${defModulePath}) 模块`);
            if (fs.existsSync(defModulePath)) {
                subModule = require(config.subscriber);
            } else {
                console.log(`subscriber (${defModulePath}) 模块不存在, 忽略.`);
            }
        }
    } catch (err) {
        console.warn(`subscriber 模块加载失败: `, err);
    }
}
